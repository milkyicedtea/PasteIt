...
pub(crate) async fn encrypt_paste(
    plain_text: &str,
    key: &Key<Aes256Gcm>,
...
pub(crate) async fn decrypt_paste(
    encrypted_text: &str,
    key: &Key<Aes256Gcm>,
) -> Result<String, (StatusCode, String)> {
    let cipher = Aes256Gcm::new(key);

    let data = hex::decode(encrypted_text).map_err(|e| {
        (
            StatusCode::BAD_REQUEST,
            "Invalid hex encoding".to_owned() + &*e.to_string(),
        )
    })?;
...
pub(crate) async fn get_key_bytes() -> Result<Vec<u8>, (StatusCode, String)> {
